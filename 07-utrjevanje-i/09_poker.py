...
def nov_kup():
    ...
    for stevilka in range(2,15):
        for znak in ["kara", "križ", "pik", "srce"]:
            kup.append((stevilka, znak))
    return kup
# =====================================================================@024229=
# 2. podnaloga
...
def razdeli_karte(igralci, karte):
    ...
    for ime in igralci:
        ...
        razdelitev[ime] = [prva_karta, druga_karta]
    return razdelitev
# =====================================================================@024231=
# 4. podnaloga
...
def tvorijo_lestvico(karte):
    ...
    stevilke.sort()
    mini = stevilke[0]
    return stevilke == list(range(mini, mini + len(stevilke)))
# =====================================================================@024234=
# 7. podnaloga
...
def kolikokrat_se_pojavi_katera_stevilka(karte):
    ...
    kolikokrat = {}
    for i in stevilke:
        kolikokrat[i] = kolikokrat.get(i, 0) + 1  # povečamo število pojavitev
    return kolikokrat
# =====================================================================@024235=
# 8. podnaloga
...
def ovrednoti(karte):
    naj = 1
    for peterka in itertools.combinations(karte, 5): # peterka je nabor petih kart
        v = vrednost(list(peterka))
        if v > naj:
            naj = v
    return naj
# =====================================================================@024237=
...
def poker(imena):
    ...
    for ime in imena:
        print("{} {} {}".format(ime, ovrednoti(razdelitev[ime] + skupne), razdelitev[ime]))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIyOCwidXNlciI6MTA3NDh9:1u3HLB:kg9Src3Rz2RXeiW4zadsF-APqi6TIHSBBSNt8eIeNS4"
        try:
            for stevilka in range(2, 15):
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIyOSwidXNlciI6MTA3NDh9:1u3HLB:f0mAKMs77xTsxQ8VRbt7unA7mUqt34FDoJHSoDFmboo"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIzMCwidXNlciI6MTA3NDh9:1u3HLB:SZwLTunrmeqraZkrbQUOCpNYPZDE9YE8b7cpxXG7qRo"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIzMSwidXNlciI6MTA3NDh9:1u3HLB:ha_9f5MzSrhJgzwF-Qvh4edOM6BvRiXtKmiKVcmTqz8"
        try:
            cases = [
                (
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIzMiwidXNlciI6MTA3NDh9:1u3HLB:wffFeUOsbXBcr0RdWDP7ARJ88YahEM8KiPxrplkfa4U"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIzMywidXNlciI6MTA3NDh9:1u3HLB:eai-r4wVIpmgpurRMswW28X4-bjYrvrHzqtdBkINeC0"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIzNCwidXNlciI6MTA3NDh9:1u3HLB:BpzSNk2Q7WVaSbfBQDHJNJLuCGUOa87uEp2mkLZv6sE"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIzNSwidXNlciI6MTA3NDh9:1u3HLB:-uNCCzT-inRrHXirDO7IQ6UEqclRh3Zr-TyF1OtCMDw"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIzNiwidXNlciI6MTA3NDh9:1u3HLB:oGer4Y3cBCSOvDkbSbF9zMAeqKUE0it9By-ckkjmORk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIzNywidXNlciI6MTA3NDh9:1u3HLB:ANwgmxkrOYYe1iTHdndFne27BsJR7tL966Y_B6JfeXA"
        try:
            pass
        except TimeoutError:
            Check.error("Dovoljen čas izvajanja presežen")
