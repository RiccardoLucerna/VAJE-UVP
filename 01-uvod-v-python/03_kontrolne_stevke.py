...
def vsota_kvadratov_stevk(n):
    ...
    desetice = (n // 10) % 10
    stotice = n // 100
    vsota = enice ** 2 + desetice ** 2 + stotice ** 2
    return vsota
# =====================================================================@000925=
# 2. podnaloga
...
def obrat(n):
    ...
    desetice = (n // 10) % 10
    stotice = n // 100
    nov_n = enice * 100 + desetice * 10 + stotice 
    return nov_n
# =====================================================================@000926=
# 3. podnaloga
...
def dodaj_kontrolno_stevko(sklic):
    enice = sklic % 10
    desetice = (sklic // 10) % 10
    stotice = (sklic // 100) % 10
    cetrta = (sklic // 1000) % 10
    peta = (sklic // 10000) % 10
    sesta = (sklic // 100000) % 10
    sedma = (sklic // 1000000) % 10
    osma = (sklic // 10000000) % 10
    deveta = (sklic // 1e8) % 10
    deseta = (sklic // 1e9) % 10
    enajsta = (sklic // 1e10) % 10
    dvanajsta = (sklic // 1e11) % 10
    kontrolna_stevka = 11 - ((enice * 2 + desetice * 3 + stotice * 4 + cetrta * 5 + peta * 6 + sesta * 7 + sedma * 8 + osma * 9 + deveta * 10 + deseta * 11 + enajsta * 12 + dvanajsta * 13) % 11)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjQsInVzZXIiOjEwNzQ4fQ:1u5mNx:P_qxLuKbWtubcc8DjDplnWfVHQNBa0Y0XxDBPMrqPjk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjUsInVzZXIiOjEwNzQ4fQ:1u5mNx:fpXERdImGK4D9cno3_oJQcFIK6mDIBupPYjbGcd4H6I"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjYsInVzZXIiOjEwNzQ4fQ:1u5mNx:k_Yk6KebJUfrvQAb-RtMY1BqZ1gZMf9ABWLDFBP_bl0"
        try:
